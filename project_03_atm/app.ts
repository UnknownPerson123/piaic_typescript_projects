...
function getUserIndex(userID: number) {
  return usersData.findIndex((user) => user.userId === userID);
}

function userAuthentication(index: number, userPin?: number) {
  if (index < 0) {
    return false;
...
async function updateBalance(user: User, amount: number, type: string) {
  const transaction = {
    type,
    amount,
    desc: "",
...
function updateTransactions(user: User, transaction: Transactions) {
...
async function transferAmount(userID: number, amount: number) {
...
async function atmOptions() {
  const atmOptions = await createCLIInputs(
    "list",
    "atm",
  ...
  if (atmOptions.atm == "Fast Cash") {
    ...
    const amount = await createCLIInputs(
      "number",
      "amount",
